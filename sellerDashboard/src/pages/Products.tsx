...
import AddNewProduct from "@/components/myComponents/AddNewProduct";

function Products() {
    const [products,setProducts] =  useState<IProductModal[] | null>(null);

    const columns:Array<keyof IProductModal> = useMemo(() => ['product_name','category_name','sub_category_name','average_rating','price','status'],[]); 

    const tableData:ICustomTable = useMemo(() =>  (
        {
            title: "Products",
            headings:  ['Product Name','Category','Sub Category','Rating','Price','Status'],
            columns: columns ,
            rowsData: products,
