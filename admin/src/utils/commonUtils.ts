...
export const isUserLogin =  () => {
    return  localStorage.getItem("ALoginToken")  ?  true  :  false;
}

export  const setUserLogin = (token:string) => {
    localStorage.setItem("ALoginToken",token);
}

export  const unsetLocalStorageVariables = ()=>{
    localStorage.removeItem("ALoginToken");
}
...
export  const getImagePathUrl   = (type:string,imageUrl:string) => {
    ...
    const imagePathObj =  {
        "main":'/api/main/v1',
        "others":'/api/others/v1',
        "page":'/api/main/v1/others',
        "user":'/api/user/v1',
