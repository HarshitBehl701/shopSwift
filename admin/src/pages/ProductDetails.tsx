...
function ProductDetails() {
    ...
    const handleProductDeleteRestore  = useCallback(async  () =>  {
      if(window.confirm(`Are you  sure  you  want to ${productData.is_active ? "'Delete'" : "'Restore'"} this product`))
      {
        try {
            const response  =  await manageProduct({product_id:productData.id,is_active:productData.is_active == 1  ? 0 : 1})  as  IApiResponse;
            if(response.status)
            {
              setProductData((prev)=>({...prev,is_active:productData.is_active == 1  ? 0 : 1}));
              handleToastPopup({message:`Successfully ${productData.is_active ? "'Delete'"  :"'Restore'"} the product`,type:"success"});
            }else
              handleToastPopup({message:(response.message),type:"error"});
        } catch (error) {
          handleToastPopup({message:handleCatchErrors(error),type:"error"});
  ...
  },[productData.id,productData.is_active]);
  ...
  return (
    <div>
      ...
      <p>Delete/Restore : <Button  variant={"link"} onClick={handleProductDeleteRestore} className={`cursor-pointer ${productData.is_active ? 'text-red-500' :  'text-green-600'}`}>{productData.is_active ?  "Delete Product" :"Restore Product"}</Button></p>
